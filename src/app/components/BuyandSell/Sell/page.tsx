...
const Sell = () => {
  ...
  useEffect(() => {
    const fetchUserData = async (email: string) => {
      try {
        const res = await fetch(`/api/getSellerDetails?email=${email}`);
        if (res.ok) {
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  try {
    ...
    const res = await fetch("/api/saveUserDetails1", {
      method: "POST",
    ...
    if (!res.ok) {
      ...
      try {
        const emailRes = await fetch("/api/sendEmail1", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      {showModal && (
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            <h2>Sell request is accepted</h2>
